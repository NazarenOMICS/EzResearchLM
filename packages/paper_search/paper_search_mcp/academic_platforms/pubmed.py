...
_NCBI_MIN_INTERVAL_SECONDS = 0.4
...
def _throttle_ncbi() -> None:
    ...
    with _NCBI_LOCK:
        ...
        if elapsed < _NCBI_MIN_INTERVAL_SECONDS:
            time.sleep(_NCBI_MIN_INTERVAL_SECONDS - elapsed)
...
class PubMedSearcher(PaperSource):
    ...
    def __init__(self):
        ...
        self.session.headers.update({
            "User-Agent": "EZresearchLM/0.1 (paper-search; PubMed E-utilities)",
    ...
    def _request_xml(self, url: str, params: dict) -> ET.Element:
        ...
        request_params.setdefault("tool", "EZresearchLM")
        ...
        for attempt in range(3):
            ...
            if response.status_code in {429, 500, 502, 503, 504}:
                last_error = RuntimeError(f"PubMed HTTP {response.status_code}")
                time.sleep(1.5 * (attempt + 1))
                continue
            if response.status_code != 200:
                raise RuntimeError(f"PubMed HTTP {response.status_code}")

            content = response.content or b""
            stripped = content.lstrip()
            if not stripped.startswith(b"<"):
                preview = response.text[:120].replace("\r", " ").replace("\n", " ")
                last_error = RuntimeError(f"PubMed returned non-XML response: {preview}")
                time.sleep(1.5 * (attempt + 1))
                continue
            ...
            try:
                ...
                preview = response.text[:120].replace("\r", " ").replace("\n", " ")
                last_error = RuntimeError(f"PubMed XML parse failed: {exc}; response starts: {preview}")
                if attempt == 2:
    ...
    def search(self, query: str, max_results: int = 10) -> List[Paper]:
        ...
        search_root = self._request_xml(self.SEARCH_URL, search_params)
        ...
        fetch_root = self._request_xml(self.FETCH_URL, fetch_params)
...
if __name__ == "__main__":
    ...
    # æµ‹è¯• PDF ä¸‹è½½åŠŸèƒ½ï¼ˆä¼šè¿”å›žä¸æ”¯æŒçš„æç¤ºï¼‰
    if papers:
    ...
    # æµ‹è¯•è®ºæ–‡é˜…è¯»åŠŸèƒ½ï¼ˆä¼šè¿”å›žä¸æ”¯æŒçš„æç¤ºï¼‰
    if papers:

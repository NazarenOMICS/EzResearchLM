...
def main():
    ...
    parser.add_argument("--output", default=None, help="Path to write passage-map JSON (never stdout)")
    ...
    out_path = args.output or f"/tmp/passage-map-{args.slug}.json"
    with open(out_path, "w", encoding="utf-8") as fout:
        json.dump(mapping, fout, ensure_ascii=False, separators=(",", ":"))
    print(f"Passage map -> {out_path}", file=sys.stderr)
    # stdout: emit only the path so callers can locate the file
    print(out_path)
